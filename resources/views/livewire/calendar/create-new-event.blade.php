...
<div class="max-w-4xl mt-2 md:mt-12 p-2 md:p-6 bg-white rounded-lg shadow-md mx-2 md:mx-auto">
    ...
    <!-- Event Creation Form -->
    <form wire:submit.prevent="createEvent" class="space-y-4 px-2 md:px-0">
        <div>
        ...
        <div class="flex h-auto w-full flex-col md:flex-row justify-between md:items-center">
            ...
            <div class="w-full md:w-1/3">
                <x-toggle id="recurring" label="Recurring?" name="recurring" wire:model.live="recurring" wire:loading.class="opacity-50 pointer-events-none"></x-toggle>
                @error('recurring')
                    <span class="text-red-500 text-xs">{{  $message }}</span>
                @enderror
            </div>
        ...
        <div class="flex h-auto w-full flex-col md:flex-row justify-start md:items-center">
            <div class="w-full md:w-1/2 pr-1 md:pr-4">
                {{-- Select the applicable recurrances instance. --}}
                <x-select parentClass="w-full" disabled="disabled" class="w-full rounded-lg border-gray-300" :label="'Select Frequency'" :options="$options"></x-select>
            </div>
        </div>
